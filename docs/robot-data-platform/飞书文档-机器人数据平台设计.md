...
第一步 · 粘贴正文
  全选本文档文字 → 复制 → 飞书文档空白处粘贴
  飞书会自动识别标题、表格、代码块、列表
  若表格未自动转换：选中区域 → 插入 → 表格

第二步 · 粘贴配图（共 9 张）
  在仓库目录 docs/robot-data-platform/images/ 打开对应 PNG
  右键复制图片（或打开后 Ctrl+C）→ 粘贴到飞书文中【图 xx】标记下方
  也可在 GitHub 预览本 Markdown，图片会在线显示，再复制到飞书

配图清单见文末「十三、配图文件清单」
图源可编辑：docs/robot-data-platform/diagrams/*.mmd（改完后运行 diagrams/render.sh 重新出图）
...
【图 01 逻辑架构】文件：images/01-architecture.png

![图01 逻辑架构](images/01-architecture.png)

说明：TiDB 只管路径与状态；ES 存检索与 tag_paths；标签树来自 YAML 配置，不进库。
...
【图 02 原始数据状态机】文件：images/02-raw-state.png

![图02 原始数据状态机](images/02-raw-state.png)
...
【图 03 资产数据状态机】文件：images/03-asset-state.png

![图03 资产数据状态机](images/03-asset-state.png)
...
用途：每条数据每个流水线任务一行，记录最新完成状态；用于统计该数据任务完成情况。

| 字段 | 说明 |
| entity_type | raw_data / asset_data |
| entity_id | 关联数据主键 |
| storage_path | 冗余路径 |
| task_code | detect/clean/preprocess/audit/synthesize/tag |
| status | pending/running/success/failed/skipped |
| sort_order | 流水线顺序 |
| started_at / finished_at | 最近执行时间 |
| duration_ms | 耗时 |
| job_id | 批任务 ID |
| attempt_count | 累计执行次数 |
| error_message | 失败原因 |

约束：UNIQUE(entity_type, entity_id, task_code)

原始数据任务序列：detect → clean → preprocess → tag
资产数据任务序列：audit → synthesize → tag

写入流程：
  数据登记 → 初始化全部 pending
  任务开始 → UPSERT status=running
  任务结束 → UPSERT status=success|failed + 写 processing_log 明细

单条数据完成度统计 SQL 见仓库：docs/robot-data-platform/appendix-task-stats.sql
...
【图 08 标签组合查询逻辑】文件：images/08-tag-query-logic.png

![图08 标签组合查询逻辑](images/08-tag-query-logic.png)
...
【图 04 TiDB ER 图】文件：images/04-er-tidb.png

![图04 TiDB ER 图](images/04-er-tidb.png)

8.2 跨存储关系（含 ES 标签）

【图 05 跨存储数据流】文件：images/05-cross-storage.png

![图05 跨存储数据流](images/05-cross-storage.png)

8.3 标签不在 DB

【图 06 标签仅存 ES 流程】文件：images/06-tag-flow.png

![图06 标签仅存 ES 流程](images/06-tag-flow.png)
...
【图 09 列表页线框】文件：images/09-ui-list-wireframe.png

![图09 列表页线框](images/09-ui-list-wireframe.png)
...
【图 07 列表检索时序图】文件：images/07-search-sequence.png

![图07 列表检索时序图](images/07-search-sequence.png)

简要步骤：
浏览器 → API：search + tag_filter → ES bool query → 返回列表
浏览器 → API：GET detail → TiDB 路径 + ES 标签 + 读 metadata 文件 → 合并详情
...
| docs/robot-data-platform/飞书文档-机器人数据平台设计.md | 本文档 |
| docs/robot-data-platform/images/ | 配图 PNG（复制到飞书） |
| docs/robot-data-platform/diagrams/ | Mermaid 图源 + render.sh |
| docs/robot-data-platform/appendix-ddl.sql | TiDB 建表 SQL |
...
| 图号 | 文件名 | 插入章节 |
| 01 | images/01-architecture.png | 二、2.1 逻辑架构 |
| 02 | images/02-raw-state.png | 三、3.1 原始数据状态机 |
| 03 | images/03-asset-state.png | 三、3.2 资产数据状态机 |
| 04 | images/04-er-tidb.png | 八、8.1 TiDB ER |
| 05 | images/05-cross-storage.png | 八、8.2 跨存储关系 |
| 06 | images/06-tag-flow.png | 八、8.3 标签不在 DB |
| 07 | images/07-search-sequence.png | 九、9.6 列表检索时序 |
| 08 | images/08-tag-query-logic.png | 七、7.2 标签组合查询 |
| 09 | images/09-ui-list-wireframe.png | 九、9.2 列表页线框 |

重新生成配图：在仓库根目录执行
  cd docs/robot-data-platform/diagrams && bash render.sh
